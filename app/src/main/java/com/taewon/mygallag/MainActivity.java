...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) { //Activity가 생성될때 실행됨
        ...
        userIntent = getIntent();   // StartActivity 액티비티에서 받아옴
        bgMusicIndex = 0;
        ...
        // SoundPool을 생성하고 효과음을 재생하기 위한 초기화 작업을 수행한다
        effectVolumn = 1;
    ...
    protected void onResume() {
        super.onResume();
        bgMusic.start();  // 일시정지 되었던 배경 음악 다시 재생
        spaceInvadersView.resume();  //  게임화면을 다시 재개
    }

    private void init() {                // 화면 너비와 높이를 얻는 방법
        Display display = getWindowManager().getDefaultDisplay();  //view의 display를 얻어온다.
        Point size = new Point();
        display.getSize(size);

        //??? 아이템 화면에 띄우기,,  spaceInvadersView 객체를 생성하고 gameFrame에 추가합니다.,,
        ...
        //음악 바꾸기
        changeBgMusic(); // 처음에 한번 음악이 나오기 위해서 필요
        ...
        effectSoundList = new ArrayList<>();   //효과음 파일의 리소스 ID를 저장하는 ArrayList를 생성하고 초기화합니다.
    ...
    private void changeBgMusic() {
        ...
        bgMusicIndex = bgMusicIndex % bgMusicList.size(); //음악 인덱스를 증가시키고, 음악 개수만큼만 바뀌게 하기 위해
    }

    @Override
    protected void onPause() {   //일시 정지시
        super.onPause();
        bgMusic.pause();   // 소리 일시정지
        spaceInvadersView.pause();  // 적군 일시정지
    }

    public static void effectSound(int flag) {
        effectSound.play(effectSoundList.get(flag), effectVolumn, effectVolumn,
                0, 0, 1.0f);
        //Soundpool 실행, 지정된 효과음을 재생
    }
